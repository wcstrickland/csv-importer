...
func main() {
	...
	for _, v := range os.Args[1:] {
		...
		query := batchString(1000, tableName, newFirstLine)
		for {
			vals := make([]interface{}, 1000*len(newFirstLine))
			record, err := r.Read()
			if err == io.EOF {
				break
			}
			...
			for i := 1; i < 1000; i++ {
				record, err := r.Read()
				if err == io.EOF {
					vals = vals[:i*len(newFirstLine)]
					query := batchString(i, tableName, newFirstLine)
